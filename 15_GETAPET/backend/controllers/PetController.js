...
module.exports = class PetController {
  ...
  static async create(req, res) {
    ...
    try {
      ...
      res.status(201).json({message: 'Pet cadastrado com seucesso', newPet})
    } catch (error) {
  ...
  static async getPetById(req, res) {
    ...
    if(!pet) {
      res.status(404).json({message: 'Pet não ecnontrado!'})
    }
  ...
  static async removePetById(req, res) {
    ...
    if(pet.user._id.toString() !== user._id.toString()) {
      res.status(404).json({message: 'Houve um problema em procesar a sua solicitação, tente novamente mais tarde!'})
      return
    }
  ...
  static async updatePet(req, res) {
    ...
    if(pet.user._id.toString() !== user._id.toString()) {
      res.status(422).json({message: 'Houve um problema em procesar a sua solicitação, tente novamente mais tarde!'})
      return
    }
  ...
  static async schedule(req, res) {
    ...
    // if(pet.user._id.toString() === user._id.toString()) {
    if(pet.user._id.equals(user._id)) { // forma mais facil do que a de cima
